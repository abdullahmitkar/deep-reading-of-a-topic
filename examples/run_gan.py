...
def mask_tokens(inputs, tokenizer, args):
    ...
    temp = torch.tensor(special_tokens_mask, dtype=torch.uint8)
    print(type(temp))
    probability_matrix.masked_fill_(temp, 0.0)
...
def train(args, train_dataset, model, tokenizer):
    ...
    for iter_a in trange(int(4), desc="Epoch", disable=args.local_rank not in [-1, 0]):
